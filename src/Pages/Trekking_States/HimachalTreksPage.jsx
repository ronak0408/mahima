...
const treksData = [
  {
    ...
    description: "One of the most popular treks in Himachal, Triund offers breathtaking views of the Dhauladhar range. It's a relatively easy trek, making it perfect for beginners.",
    length: "9 km (one way)",
...
// --- SVG Icons (using inline SVG for simplicity) ---
...
// Banner Component
const Banner = () => (
  <div className="relative h-[50vh] bg-cover bg-center" style={{ backgroundImage: "url('https://www.naturetravelagency.com/uploads/1724499766best%20time%20to%20visit%20himachal%20Pradesh%20for%20snowfall.png')" }}>
    <div className="absolute inset-0 bg-black/65 flex items-center justify-center">
      <div className="text-center text-white p-4">
        <h1 className="text-4xl md:text-6xl font-bold tracking-tight">Himachal Trekking Adventures</h1>
        <p className="mt-4 text-lg md:text-xl max-w-2xl">Discover the most breathtaking trekking sites in the heart of the Himalayas.</p>
      </div>
    </div>
  </div>
);
...
const TrekCard = ({ trek, onSelect }) => (
  ...
  >
    ...
    <div className="p-6">
      <h3 className="text-2xl font-bold text-gray-800 group-hover:text-cyan-600 transition-colors duration-300">{trek.name}</h3>
        ...
        <div className="w-full lg:w-1/2 p-6 md:p-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center text-lg">
              <RouteIcon className="w-6 h-6 mr-3 text-cyan-600 flex-shrink-0" />
              <div>
            ...
            <div className="flex items-center text-lg">
              <MapPinIcon className="w-6 h-6 mr-3 text-cyan-600 flex-shrink-0" />
              <div>
    ...
    <Navbar></Navbar>
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {treksData.map((trek) => (
            <TrekCard key={trek.id} trek={trek} onSelect={handleSelectTrek} />
          ))}
        </div>
      </main>

      <TrekDetailModal trek={selectedTrek} onClose={handleCloseModal} />
